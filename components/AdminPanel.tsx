...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [editingForm, setEditingForm] = React.useState<Partial<Product> | null>(null);

  React.useEffect(() => {
    if (editingProductId) {
      const p = products.find(prod => prod.id === editingProductId);
      if (p) setEditingForm({ ...p });
    } else {
      setEditingForm(null);
    }
  ...
  const handleSaveEdit = () => {
    if (editingForm && editingForm.id) {
      // Recalculate price if MRP or Off changed
      const mrp = editingForm.mrp || 0;
      const off = editingForm.off || 0;
      const price = Math.floor(mrp * (1 - off / 100));

      onUpdateProduct({ ...editingForm, price } as Product);
  ...
  return (
    <div className="pt-4 pb-20">
              ...
              <div key={p.id} className="bg-white/5 border border-white/5 rounded-xl p-3 flex flex-col gap-3 relative group">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-white/40 text-xs">{p.brand} • MRP: {p.mrp} • <span className="text-pink-400">{p.price}</span></p>
                  </div>
      ...
      />
      {/* EDIT MODAL */}
      {editingProductId && editingForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-[#1e293b] border border-white/10 rounded-2xl w-full max-w-lg p-6 shadow-2xl overflow-y-auto max-h-[90vh]">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-white">Edit Product</h3>
              <button onClick={() => setEditingProductId(null)} className="text-white/40 hover:text-white"><X size={24} /></button>
            </div>

            <div className="space-y-4">
              {/* Image Edit */}
              <div className="flex justify-center mb-6">
                <div className="relative w-32 h-32 rounded-xl overflow-hidden bg-black/40 border border-white/10 group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                  {editingForm.image ? <img src={editingForm.image} className="w-full h-full object-cover" alt="Product" /> : <div className="flex items-center justify-center h-full text-white/20"><Camera size={32} /></div>}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs text-white/40 uppercase tracking-wider mb-1 block">Product Name</label>
                  <input type="text" value={editingForm.name} onChange={e => setEditingForm({ ...editingForm, name: e.target.value })} className="w-full bg-black/20 text-white rounded-lg p-3 text-sm border border-white/5" />
                </div>
                <div>
                  <label className="text-xs text-white/40 uppercase tracking-wider mb-1 block">Brand</label>
                  <input type="text" value={editingForm.brand} onChange={e => setEditingForm({ ...editingForm, brand: e.target.value })} className="w-full bg-black/20 text-white rounded-lg p-3 text-sm border border-white/5" />
                </div>
              </div>

              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="text-xs text-white/40 uppercase tracking-wider mb-1 block">MRP</label>
                  <input type="number" value={editingForm.mrp} onChange={e => setEditingForm({ ...editingForm, mrp: parseInt(e.target.value) || 0 })} className="w-full bg-black/20 text-white rounded-lg p-3 text-sm border border-white/5" />
                </div>
                <div>
                  <label className="text-xs text-white/40 uppercase tracking-wider mb-1 block">Discount %</label>
                  <input type="number" value={editingForm.off} onChange={e => setEditingForm({ ...editingForm, off: parseInt(e.target.value) || 0 })} className="w-full bg-black/20 text-white rounded-lg p-3 text-sm border border-white/5" />
                </div>
                <div>
                  <label className="text-xs text-white/40 uppercase tracking-wider mb-1 block">Size</label>
                  <input type="text" value={editingForm.size} onChange={e => setEditingForm({ ...editingForm, size: e.target.value })} className="w-full bg-black/20 text-white rounded-lg p-3 text-sm border border-white/5" />
                </div>
              </div>

              <div>
                <label className="text-xs text-white/40 uppercase tracking-wider mb-1 block">Category</label>
                <select
                  className="w-full bg-black/20 text-white rounded-lg p-3 text-sm outline-none border border-white/5"
                  value={editingForm.category} onChange={e => setEditingForm({ ...editingForm, category: e.target.value })}
                >
                  <option>Hair Care</option>
                  <option>Skin Care</option>
                  <option>Makeup</option>
              ...
              <div>
                ...
                <textarea value={editingForm.desc} onChange={e => setEditingForm({ ...editingForm, desc: e.target.value })} className="w-full bg-black/20 text-white rounded-lg p-3 text-sm border border-white/5 h-24 resize-none"></textarea>
              </div>

              <button onClick={handleSaveEdit} className="w-full py-4 bg-pink-500 hover:bg-pink-600 rounded-xl text-white font-bold text-lg shadow-lg mt-4">Save Changes</button>
            </div>
          </div>
        </div>
      )}
